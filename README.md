...
- 🔀 **转发规则** — TCP/UDP 端口转发，多目标、故障转移与轮询负载均衡，长链路低延迟、低抖动
- 🛡️ **目标熔断** — 单目标连续失败自动跳过一段时间，全部熔断时自动试探恢复
- 🌐 **域名目标 & DDNS 跟随** — 目标可填域名，DDNS 换 IP 后自动跟随，无需手动重启规则或节点
- ♻️ **高并发连接稳定** — 自动回收掉线残留的死连接，撑住长期高并发
- 🚦 **规则连接数上限** — 每条规则可单独限制并发连接数，超出的新连接直接拒绝（TCP）
- 🔄 **规则重启** — 单条重启、勾选批量重启，或设定时自动重启；重启会掐断该规则上已建立的连接并重建监听
- 🛒 **套餐商城与计费** — 用户自助购买（余额扣费）、查看订单；管理员配置套餐（增删改），套餐绑定线路并在购买时自动授权
- 💳 **卡密充值** — 管理员批量生成卡密，用户在个人中心或商店自助兑换充值，无需接支付接口
...
- 📊 **流量趋势图** — 近 1 天 / 7 天 / 30 天用量曲线，按线路分色堆叠，看得出是哪条线路在吃额度
- 🔔 **节点掉线告警** — 节点失联超过设定时长推送 Telegram 或邮件通知，恢复后再推一条
...
- ⬆️ **节点一键升级** — 面板下发升级，节点从官方 Release 自更新，只升不降，无需登录节点
- 🖱️ **规则极简导入/导出** — 支持批量导入、批量启停并自动下发
...
**节点更新：面板 → 节点状态 → 对每个节点点「升级」即可**，不用 SSH 登录节点。
面板会把升级指令下发给节点，节点自己从官方 Release 拉取新版本（校验 sha256、
只升不降）。升级会断开该节点上正在进行的转发连接。

> **一键升级只对 systemd 节点可用** —— 自升级靠「旧进程退出后由进程管理器拉起
> 新进程」。Docker 节点会提示「请更新镜像」（拉最新镜像重建容器）；手动前台运行
> 的节点退出后没有东西拉起它，按钮是灰的。

手动升级（一键升级不可用时）：面板 **设备分组 → 复制对接命令** → 粘贴到节点执行。
详见 [转发节点文档](docs/NODE.zh-CN.md#更新)。
